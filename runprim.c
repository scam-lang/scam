...
defineFunction(int name,int parameters,int body,int env)
    ...
    assureMemory("defineFunction",CLOSURE_CELLS + DEFINE_CELLS + 2*RUNNER_CELLS + 1,
        &name,&parameters,&body,&env,(int *) 0);
...
define(int args)
    ...
    if (type(first) == RUNNER)
        {
        first = extractRunner(first);
...
begin(int args)
    {
    //printf("in begin...\n");
    return runList(cadr(args),car(args),ALLBUTLAST);
    }
...
scope(int args)
    ...
    return runList(cadr(args),env,ALLBUTLAST);
    }
...
cond(int args)
    ...
    while (cases != 0)
        ...
        if (sameSymbol(result,trueSymbol))
            return  runList(cdar(cases),env,ALLBUTLAST);
...
wwhile(int args)
    ...
    push(args);
    testResult = run(cadr(args),car(args));
    args = pop();
    ...
    while (sameSymbol(testResult,trueSymbol))
        {
        push(args);
        last = runList(caddr(args),car(args),ALL);
        args = pop();
