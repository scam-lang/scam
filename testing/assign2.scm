...
(define (run9)
    (inspect ((dot ((dot (Polar 3 1.423) add) (Polar 7 2.453)) toString)))
    ;(inspect (+ (rectangular 3 1.423) (rectangular 7 2.453)))
    ;(inspect (+ (polar 3 1.423) 0))
    ;(inspect (+ (rectangular 3 1.423) 0))
    )
...
(define (Polar m a) 
    (define (add a)
        (cond
            ((is? a 'Integer) ((dot ((dot (convert) add) a) convert)))
            ((is? a 'Real) ((dot ((dot (convert) add) a) convert)))
            ((is? a 'Rectangular) ((dot ((dot a add) __context) convert)))
            (else ((dot ((dot (convert) add) a) convert)))
            )
        )
    (define (convert)
    ...
    (define (promote) __context)
    (define (demote) (Real (car ((dot (convert) value)))))
